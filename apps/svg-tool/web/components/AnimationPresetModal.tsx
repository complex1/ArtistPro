...
export function AnimationPresetModal({
  ...
  const [config, setConfig] = useState(() => configForPreset('fade-in'))
  const [openCategory, setOpenCategory] = useState<PresetCategoryId | null>(
    presetById('fade-in')?.category ?? 'entrance',
  )
  const [query, setQuery] = useState('')
  const [confirming, setConfirming] = useState(false)
  const modalRef = useRef<HTMLElement>(null)
  const preset = presetById(presetId)
  const ready = preset ? presetIsReady(preset, node) : false
  const conflicts = ready
    ? presetConflicts(animation, node.id, presetId, config)
    : []
  ...
  const grouped = useMemo(() => {
    ...
    return PRESET_CATEGORIES.map((item) => ({
      ...item,
      presets: ANIMATION_PRESETS.filter((presetItem) => {
        if (presetItem.category !== item.id) return false
        if (!needle) return true
        return (
          presetItem.name.toLowerCase().includes(needle) ||
          presetItem.description.toLowerCase().includes(needle)
        )
      }),
    })).filter((item) => item.presets.length > 0)
  }, [query])

  const toggleCategory = (id: PresetCategoryId) => {
    setOpenCategory((current) => (current === id ? null : id))
  }

  const selectPreset = (id: AnimationPresetId) => {
    const category = presetById(id)?.category
    if (category) setOpenCategory(category)
    setPresetId(id)
    setConfig(configForPreset(id))
    setConfirming(false)
  }
  ...
  const unavailableReason = !preset
    ? 'This preset is missing from the catalog.'
    : preset.planned || preset.recipe.kind === 'planned'
      ? 'This preset is listed for later and is not applied yet.'
      : preset.requires?.includes('path') && node.type !== 'path'
        ? 'Select a path layer to use this preset.'
        : preset.requires?.includes('text') && node.type !== 'text'
          ? 'Select a text layer to use this preset.'
          : ''
      ...
      >
        ...
        <div className="preset-modal-body">
          <nav className="preset-list" aria-label="Animation presets">
            <div className="preset-list-toolbar">
              <input
            ...
            {grouped.map((group) => {
              const expanded = searching || openCategory === group.id
              return (
                <div
                  key={group.id}
                  className={`preset-group${expanded ? ' is-open' : ''}`}
                >
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <em>{group.presets.length}</em>
                  </button>
                  {expanded && (
                    <div
                      className="preset-group-items"
                    ...
                    >
                      {group.presets.map((item) => (
                        <button
                          ...
                          className={[
                            'preset-item',
                            item.id === presetId ? 'is-selected' : '',
                            item.planned ? 'is-planned' : '',
                          ]
                            .filter(Boolean)
                            .join(' ')}
            ...
            {grouped.length === 0 && (
              <p className="preset-empty">No presets match that search.</p>
            )}
          </nav>
          ...
          <div className="preset-preview-column">
            ...
            <div className="preset-preview-caption">
              <strong>{preset?.name ?? 'Unknown preset'}</strong>
              <span>{preset?.description ?? unavailableReason}</span>
            </div>
          </div>
          ...
          <aside className="preset-config">
            ...
            {unavailableReason && (
              <p className="preset-unavailable">{unavailableReason}</p>
            )}
            <PresetControls
              preset={preset}
              config={config}
...
function PresetPreview({
  ...
  const proxy = useMemo(() => {
    ...
    shape.transform.position = { x: 0, y: 0 }
    shape.transform.pivot = { x: 45, y: 45 }
    if (shape.type === 'rect') {
      shape.width = 90
      shape.height = 90
    }
  ...
  const animation = useMemo<DocumentAnimation>(
    () => ({
      ...
      tracks: ready ? buildPresetTracks(proxy, presetId, config, 0) : [],
    }),
    [config, presetId, proxy, ready],
  )
...
function PresetControls({
  ...
  const controls = preset?.controls ?? ['duration', 'easing']
  return (
    <div className="preset-controls">
      ...
      {controls.includes('slideDirection') && (
      ...
      {controls.includes('spinDirection') && (
        <PresetField label="Direction">
          <Select
      ...
      {controls.includes('bounceHeight') && (
        <PresetNumber
          label="Height"
          fieldLabel="PX"
