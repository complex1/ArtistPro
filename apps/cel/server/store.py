...
def _projects_root() -> Path:
    return resolve_data_root() / "cel" / "projects"
        ...
        summaries.append(
            {
                ...
                "width": document.get("width", 0) if isinstance(document, dict) else 0,
                "height": document.get("height", 0) if isinstance(document, dict) else 0,
            }
        )
...
def write_project(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(document, dict) or document.get("version") != 1:
        raise StoreError(400, "cel document version 1 required")
