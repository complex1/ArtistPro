...
beforeEach(() => {
  ...
  vi.stubGlobal(
    ...
    async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const projectsUrl = /\/v1\/apps\/cel\/projects\/?$/
      const oneUrl = /\/v1\/apps\/cel\/projects\/([^/?]+)$/
...
describe('cel project library', () => {
  it('creates, lists, updates, and deletes projects via the API', async () => {
    const created = await createProject('Still')
    expect(created.document.name).toBe('Still')
    expect(created.document.version).toBe(1)
    expect(await listProjects()).toEqual([
      expect.objectContaining({ id: created.id }),
    ])

    created.document.name = 'Still v2'
    records.set(created.id, created)
    expect((await getProject(created.id))?.document.name).toBe('Still v2')
