...
export async function runInbetweenChecks(window) {
  ...
  await check(`document.querySelector('[aria-label="Line motion refinement"]')?.value === 'adaptive' && document.querySelector('.fbf-ib-local')?.textContent.includes('ML assist')`, 'The local ML motion model is selected by default and its on-device status is visible')
  await change('Line motion refinement', 'guided')
  await check(`document.querySelector('.fbf-ib-local')?.textContent.includes('Guided')`, 'Guided TPS remains available as an explicit refinement fallback')
  await change('Line motion refinement', 'adaptive')
  await key('n')
