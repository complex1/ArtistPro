...
import { createDocument, validateDocument, type AnimationDocument } from './model'

const DATABASE = 'artist-studio-frame-by-frame'
...
function openDatabase(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    if (typeof indexedDB === 'undefined') {
      reject(new Error('Browser storage is unavailable. Enable IndexedDB to save FrameByFrame projects.'))
      return
    }
    let blocked = false
    ...
    request.onerror = () => reject(request.error ?? new Error('Could not open FrameByFrame storage.'))
    request.onblocked = () => {
      blocked = true
      reject(new Error('Close other FrameByFrame windows, then try again.'))
    }
  })
}
...
export async function listProjects(): Promise<ProjectSummary[]> {
  ...
  return records.map((record) => {
    ...
    return {
      ...
      width: document.width, height: document.height, layerCount: document.layers.length, fps: document.fps, duration: document.duration,
    }
...
export async function createProject(name = 'Untitled shot', width = 1280, height = 720, fps = 12): Promise<ProjectRecord> {
  const now = Date.now()
  return saveProject({ id: nanoid(), createdAt: now, updatedAt: now, document: createDocument(name, width, height, fps) })
}
