...
export function stampPaintSrc(stamp: string): string {
  return isShapeStamp(stamp) ? stamp.slice(SHAPE_STAMP_PREFIX.length) : stamp
}
...
// Dark pixels become opaque stamp coverage; invert flips that. Original alpha
// is kept so holes in a PNG stay holes.
